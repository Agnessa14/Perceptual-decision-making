function fixed_analysis_pilot_2(subjects) %distance art, distance nat, RT
%FIXED_ANALYSIS_PILOT_2 Performs the distance-to-hyperplane analysis using
%the LDA classifier 60 scenes on normalized distances.
%
...
for subject = subjects
    ...
    load(fullfile(results_dir,subname,'dth_lda.mat'));
    distances(subject,:,:) = averaged_decision_values;   
    load(fullfile(results_dir,subname,'RTs_correct_answers.mat'));
...
save(fullfile(save_path,sprintf('DTH_rt_correlation_both_categories_%d_subjects',numel(subjects))),'correlation_dth_rt_both');
save(fullfile(save_path,sprintf('DTH_rt_correlation_artificial_%d_subjects',numel(subjects))),'correlation_dth_rt_art');
save(fullfile(save_path,sprintf('DTH_rt_correlation_natural_%d_subjects',numel(subjects))),'correlation_dth_rt_nat');
save(fullfile(save_path,sprintf('DTH_rt_correlation_both_categories_avg_%d_subjects',numel(subjects))),'correlation_dth_rt_avg');

%figures
saveas(gcf,fullfile(save_path,sprintf('DTH_artificial_natural_%d_subjects',numel(subjects))));
saveas(gcf,fullfile(save_path,sprintf('DTH_artificial_natural_%d_subjects.svg',numel(subjects))));
