function dth_subject_by_subject(subjects,task_distance,task_RT,with_stats) %distance art, distance nat, RT
%DTH_SUBJECT_BY_SUBJECT Performs the distance-to-hyperplane analysis using
%the distances-to-hyperplane and the RT for each subject separately.
%
%Input: subjects' ID (e.g., 1:13), task for the distances (1=categorization, 2=distraction), task for RT,
%add stats (1 with, 0 without)
%
%Correlates the decision values and RTs of each subject, averaged, at each timepoint, resulting in a plot of Spearman's correlation vs time. 
%
...
%% Correlate each subject's distances with their RTs
t = 1:numTimepoints;
...
for subject = subjects
    correlation_art(subject,:) = arrayfun(@(x) corr(squeeze(distances(subject,artificial_conditions,x))',squeeze(RTs_art(subject,:))','type','Spearman'), t);
    correlation_nat(subject,:) = arrayfun(@(x) corr(squeeze(distances(subject,natural_conditions,x))',RTs_nat(subject,:)','type','Spearman'), t);
    correlation_both(subject,:) = arrayfun(@(x) corr(squeeze(distances(subject,:,x))',RTs(subject,:)','type','Spearman'), t);
...
if isequal(task_distance,task_RT)
    save(fullfile(save_path,sprintf('cv_fully_random_effects_dth_subjects_%d_%d_%s.mat',subjects(1),subjects(end),task_name_distance)),'dth_results');
    saveas(gcf,fullfile(save_path,sprintf('cv_fully_random_effects_dth_subjects_%d_%d_%s',subjects(1),subjects(end),task_name_distance))); 
    saveas(gcf,fullfile(save_path,sprintf('cv_fully_random_effects_dth_subjects_%d_%d_%s.svg',subjects(1),subjects(end),task_name_distance))); 
else
    save(fullfile(save_path,sprintf('cv_fully_random_effects_dth_subjects_%d_%d_cross_task_%s_distances.mat',subjects(1),subjects(end),task_name_distance)),'dth_results');
    saveas(gcf,fullfile(save_path,sprintf('cv_fully_random_effects_dth_subjects_%d_%d_cross_task_%s_distances',subjects(1),subjects(end),task_name_distance)));
    saveas(gcf,fullfile(save_path,sprintf('cv_fully_random_effects_dth_subjects_%d_%d_cross_task_%s_distances.svg',subjects(1),subjects(end),task_name_distance)));
end
