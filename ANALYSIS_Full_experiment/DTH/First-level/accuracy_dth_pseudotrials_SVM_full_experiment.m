function accuracy_dth_pseudotrials_SVM_full_experiment(subject,task)
%ACCURACY_DTH_PSEUDOTRIALS_SVM_FULL_EXPERIMENT Performs the distance-to-hyperplane analysis using SVM on
...
%
%Output: 
%   - NxP matrix of decision values (signed), where N is the number of conditions
%   and P is the number of timepoints.
%   - 1xP matrix of decoding accuracies 
%
...
predictedLabels = NaN(numPermutations,numConditions,numTimepoints);
...
for perm = 1:numPermutations
    ...
    for t = 1:numTimepoints
        ...
        disp('Test the SVM');
        [prediction, accuracy, decision_values] = svmpredict(labels_test,testing_data,model);  
        ...
        predictedLabels(perm,:,t) = prediction;
...
predictedLabels_Avg = squeeze(mean(predictedLabels,1));
classification_results = struct('decisionValues',decisionValues_Avg,'decodingAccuracy',decodingAccuracy_Avg,...
    'predictedLabels',predictedLabels_Avg);
save(fullfile(results_dir,sprintf('classification_results_dth_%s',task_name)),'classification_results');

% save(fullfile(results_dir,sprintf('noabs_dth_pseudotrials_svm_decisionValues_%s',task_name)),'decisionValues_Avg');
% save(fullfile(results_dir,sprintf('noabs_dth_pseudotrials_svm_decodingAccuracy_%s',task_name)),'decodingAccuracy_Avg');
% save(fullfile(results_dir,sprintf('noabs_dth_pseudotrials_svm_predictedLabels_%s',task_name)),'predictedLabels_Avg');
