...
def bl_net(input_tensor, classes_scenes, n_timesteps=8, cumulative_readout=False):
...
#                 kernel_regularizer=tf.keras.regularizers.l2(1e-2), #1e-6
#                 name='ReadoutDense')

        readout_dense_scenes = tf.keras.layers.Dense(
            classes_scenes, kernel_initializer='glorot_uniform',
            kernel_regularizer=tf.keras.regularizers.l2(1e-2), #1e-6
            bias_initializer='zeros',
            name='ReadoutDense_Scenes')
...
#             presoftmax[t] = readout_dense(x)
            presoftmax[t] = readout_dense_scenes(x)
