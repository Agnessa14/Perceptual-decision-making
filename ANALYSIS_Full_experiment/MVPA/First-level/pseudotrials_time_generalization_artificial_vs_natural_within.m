function pseudotrials_time_generalization_artificial_vs_natural_within(subject,task) 
%PSEUDOTRIALS_TIME_GENERALIZATION_ARTIFICIAL_VS_NATURAL_WITHIN Perform category decoding
...
for perm = 1:numPermutations
    ...
    for tp1 = 1:numTimepoints 
        ...
        training_data = [squeeze(bins(1,1:end-testing_bins,:,tp1)); squeeze(bins(2,1:end-testing_bins,:,tp1))];  %train on all but one bin                  
        ...
        for tp2 = 1:numTimepoints
            disp('Test the SVM');
            testing_data  = [squeeze(bins(1,end-testing_bins+1:end,:,tp2)); squeeze(bins(2,end-testing_bins+1:end,:,tp2))];
...
save(fullfile(results_dir,subname,sprintf('timegen_pseudotrials_svm_artificial_vs_natural_decoding_accuracy_%s.mat',task_name)),'timeg_decodingAccuracy_avg');
