function plot_object_decoding_full_experiment(subjects,task,analysis)
...
for subject = subjects
    ...
    if strcmp(analysis,'object_decoding')
        decodingAccuracy_avg = squeeze(nanmean(nanmean(decodingAccuracy_avg,1),2));
    end
...
title = sprintf('%s decoding per timepoint for %d subjects in a %s task',analysis_title,numel(subjects),task_title);
onset_time = 40; 
...
elseif strcmp(analysis,'category_decoding')
    saveas(gcf,fullfile(results_avg_dir,sprintf('svm_artificial_vs_natural_%d_subjects_%s',numel(subjects),task_name))); %save as matlab figure
    saveas(gcf,fullfile(results_avg_dir,sprintf('svm_artificial_vs_natural_%d_subjects_%s.svg',numel(subjects),task_name))); %save as svg    
end
close(gcf)
end