...
mainDirectoryEEG_1 = dir(fullfile(data_path,subname,'/EEG/','*.eeg')); 
fileNameEEG_1 = fullfile(data_path, subname,'/EEG/', mainDirectoryEEG_1.name); 
mainDirectoryBeh_1 = dir(fullfile(data_path,subname,'/Behavioural/','*.mat')); 

%% Fix order of blocks since they do not load in the correct order
filenamesAll = extractfield(mainDirectoryBeh_1,'name');
for m = 1:numel(mainDirectoryBeh_1)  
    parsedName = split(filenamesAll{m},'_');
    parsedBlocknum = split(parsedName{4},'.');
    mainDirectoryBeh_1(m).blockNum = str2double(parsedBlocknum{1});     
end

%reorder by block number
T = struct2table(mainDirectoryBeh_1); 
sortedT = sortrows(T, 'blockNum'); 
mainDirectoryBeh_1 = table2struct(sortedT); 
filenamesSorted = extractfield(mainDirectoryBeh_1,'name');

%double check that the order is correct
blocks = 1:numel(mainDirectoryBeh_1);
blockorderarray = arrayfun(@(x) mainDirectoryBeh_1(x).blockNum,blocks);
