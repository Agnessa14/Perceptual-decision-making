...
%% Load the RTs
num_subjects = numel(subjects)/2; %included subjects are only half (the rest were used for cross validation)
filename = 'RTs_shuffled_across_subjects';
load(fullfile(results_avg_dir,filename),'data');
RTs_shuffled = data(num_subjects+1:end,:);
...
for s = 1:num_subjects
    for c = 1:3
        ...
        leftout_subject = squeeze(RTs_shuffled(s,conds));
        other_subjects = squeeze(nanmean(RTs_shuffled(1:end~=s,conds),1)); %avg over remaining subjects
...
save(fullfile(results_avg_dir,sprintf('lower_bound_noise_ceiling_%s',filename)),'noise_ceiling_lower_bound');
